...
@route_users.get(
    "/all/", summary="Get all Customers", response_model=List[UserCustomerOut]
)
...
@route_users.get("/get/{id}/", summary="Get Customer", response_model=UserCustomerOut)
def read_customer(
    id: int,
    db: Session = Depends(get_db),
...
@route_users.delete("/delete/{id}/", summary="Delete Customer")
def delete_customer(
    id: int,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
